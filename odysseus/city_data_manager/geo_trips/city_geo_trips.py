		...
		if len(self.trips):
			...
			self.trips["geometry"] = self.trips_df_norm.apply(
				lambda row: LineString([
					Point(row["start_longitude"], row["start_latitude"]),
					Point(row["end_longitude"], row["end_latitude"]),
				]), axis=1
			)
			...
			self.trips_origins["geometry"] = self.trips_origins.apply(
				lambda row: Point(row["start_longitude"], row["start_latitude"]), axis=1
			)
			self.trips_destinations["geometry"] = self.trips_destinations.apply(
				lambda row: Point(row["end_longitude"], row["end_latitude"]), axis=1
			)
