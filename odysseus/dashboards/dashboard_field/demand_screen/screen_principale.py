...
from odysseus.dashboards.dashboard_field.demand_screen.chart_temp import ChartTemp
...
class ScreenDemand(DashboardScreen):
    def __init__(self, title, name, month, year, city, result_path, db="big_data_db", chart_list=None, subtitle=None, widget_list = None):
        ...
        if result_path is None:
            st.error("No simulation has been run, see you then!")
            st.stop()
        else:
        ...
        #param_list = get_simulation_names(self.result_path)
        ...
        self.widget_list = [partial(st_functional_columns, [['date_input', "Inserisci la data di inizio analisi", _min, _min, _max], 
                                                            ...
                                                            #[ "selectbox", "Scegli il nome dello scenario", param_list]])]
    ...
    def show_charts(self):
        ...
        if start>=end:
            ...
            #sim_booking_requests = pd.read_csv(os.path.join(self.result_path , path_to_results, 'sim_booking_requests.csv'))
            #sim_booking_requests_model, sim_booking_requests_trace = [f.name for f in os.scandir(self.result_path) if f.name.startswith(path_to_results)]

            sim_booking_requests_model = pd.read_csv(os.path.join(self.result_path, 'sim_booking_requests.csv'))
            #sim_booking_requests_trace = pd.read_csv(os.path.join(self.result_path, sim_booking_requests_trace, 'sim_booking_requests.csv'))

            ChartTemp(sim_booking_requests_model, sim_booking_requests_model, 'Booking requests Stats', 'Here we show statistics regarding the booking requests generated by the simulations run. Chose the statistic you are most interesteed in and browse through the simulations with the sidebar widgets').show_statistics()
