...
from e3f2s.city_data_manager.city_data_source.trips_data_source.austin_scooter_trips import AustinScooterTrips
from e3f2s.city_data_manager.city_data_source.geo_data_source.austin_census_tracts import AustinCensusTracts
...
class AustinGeoTrips(CityGeoTrips):
    ...
    def __init__(self, city_name="Austin", trips_data_source_id="city_of_austin", year=2019, month=8):

        self.city_name = city_name
        super().__init__(self.city_name, trips_data_source_id, year, month)
        self.trips_ds_dict = {
            "city_of_austin": AustinScooterTrips()
        }
        self.trips_df_norm = pd.DataFrame()

        self.census_tracts_ds = AustinCensusTracts()
